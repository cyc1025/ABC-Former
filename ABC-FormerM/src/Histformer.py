    ...
    def forward(self, x, mask=None):
        ...
        for blk in self.blocks:
            if self.use_checkpoint:
                ...
                x, y = blk(x,mask)
                Weight.append(y.permute(0,2,1).detach())

        return x, Weight


########### Histoformer ################
class Hist_Histoformer(nn.Module): #[2, 2, 2, 2, 2, 2, 2, 2, 2] [1, 2, 8, 8, 8, 8, 8, 2, 1]
    def __init__(self, in_chans=3, embed_dim=32,
        ...
        super().__init__()
        self.conv_input = nn.Conv1d(in_chans, wbset_num, kernel_size=1, padding=0)
        ...
        # Input/Output
        self.input_projection = InputProjection(in_channel=wbset_num, out_channel=embed_dim, kernel_size=3, stride=1, act_layer=nn.LeakyReLU)
        self.output_projection = OutputProjection(in_channel=embed_dim, out_channel=wbset_num, kernel_size=3, stride=1)
    ...
    def forward(self, x, mask=None):
        ...
        cha_hist=[d0,d1,d2,d3,d4,d5,d6]
        hist_feaure = [deconv3.detach(),conv4.detach(),conv0.detach()]
        # print('hist_out', x_y.shape)
        # print('..............')
