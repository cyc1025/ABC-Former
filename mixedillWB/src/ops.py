...
def histogram_loaderpatch(image, lab_state=False):
    if lab_state==True:
...
def histogram_loadertest(image, lab_state=False):
    if lab_state==True:
      image = image.astype(np.uint8)
    else:
      image = (image*255).astype(np.uint8)
...
def RGBtoLab(image):
    ...
    for i in range(b):
      imagep = image[i,:,:,:]
      lab_imagep = cv2.cvtColor(imagep, cv2.COLOR_BGR2Lab)
      lab_images.append(lab_imagep)
    lab_images = np.stack(lab_images, axis=0)
    return lab_images #bhwc

def RGBtoLab_test(image):
    image = (image*255).astype(np.uint8)
    h,w,c = image.shape
